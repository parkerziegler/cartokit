<script lang="ts">
	...
	$: size =
		$selectedLayer && isDotDensityLayer($selectedLayer) ? $selectedLayer.style.dots.size : 0;
	$: dotValue =
		$selectedLayer && isDotDensityLayer($selectedLayer) ? $selectedLayer.style.dots.value : 1;
